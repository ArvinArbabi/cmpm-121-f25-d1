...
function buildAppSkeleton(): HTMLDivElement {
  ...
  root.append(title, flavor);

  return root;
}

function buildClickButton(root: HTMLElement): HTMLButtonElement {
  const btn = el("button", { class: "big" }) as HTMLButtonElement;
...
function buildCounterPanel(root: HTMLElement): {
  clipsLabel: HTMLDivElement;
  rateLabel: HTMLDivElement;
} {
  const panel = el("div", { class: "panel" });
  const row = el("div", { class: "row" });
  const clipsLabel = el("div", { class: "mono" });
  const rateLabel = el("div", { class: "mono" });
  row.append(clipsLabel, rateLabel);
  panel.appendChild(row);
  root.appendChild(panel);
  return { clipsLabel, rateLabel };
}

function buildShop(root: HTMLElement, list: Item[]): void {
  const panel = el("div", { class: "panel grid" });
  const header = el("div");
  header.innerHTML = "<b>Upgrades</b> (buy to increase clips/sec)";
  ...
  for (const it of list) {
    ...
    const btn = el("button", { text: "Buy" }) as HTMLButtonElement;
...
// Build UI (top-down, immutable references)
injectStyle();
const appRoot = buildAppSkeleton();
const clickBtn = buildClickButton(appRoot);
const { clipsLabel, rateLabel } = buildCounterPanel(appRoot);
buildShop(appRoot, items);

// ====================================
// Game Logic (state, handlers, UI updates)
// ====================================
let clips = 0;
const clickValue = 1;
let clipsPerSecond = 0;
...
function handleBuy(it: Item) {
  if (clips >= it.cost) {
    clips -= it.cost;
    it.owned += 1;
    clipsPerSecond += it.rate;
    it.cost = +(it.cost * PRICE_MULTIPLIER).toFixed(2);
    updateUI();
  }
}

// Attach purchase handlers after UI exists
for (const it of items) {
  buyButtons[it.id].addEventListener("click", () => handleBuy(it));
}

function updateUI() {
  clipsLabel.textContent = `🧮 ${fmt(clips)} paperclips`;
  rateLabel.textContent = `• ${fmt(clipsPerSecond)} clips/sec`;
